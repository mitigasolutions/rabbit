...
declare -A lastModifications
currentTime=$(date +%s)
for autoscaler in "${autoscalingArr[@]}"; do
  lastModifications["$autoscaler"]=$currentTime
done

while true; do
  currentTime=$(date +%s)
  for autoscaler in "${autoscalingArr[@]}"; do
    IFS='|' read minPods maxPods mesgPerPod namespace deployment queueName scaleDownDelay <<< "$autoscaler"
      ...
      if [[ $requiredPods != "" ]]; then
        ...
        if [[ $currentPods != "" ]]; then
          if [[ $requiredPods -ne $currentPods ]]; then
            ...
            # If scaleDownDelay did't pass, avoid scaling down
            if [[ $requiredPods -le $currentPods ]]; then
              lastModification=lastModifications["$autoscaler"]
              threshold="$((lastModification+scaleDownDelay))"
              if [[ $currentTime -le $threshold ]]; then
                scale=1
              fi
            fi
